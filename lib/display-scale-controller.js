...
export class DisplayScaleController {
  constructor(settings, gettext) {
    ...
    this._proxy = new Promise((resolve, reject) => {
  ...
  async setScale(scale, { persistent = false } = {}) {
    ...
    if (typeof scale !== "number" || !Number.isFinite(scale) || scale <= 0)
      throw new RangeError(this._(`The scale ${scale} is invalid.`));
    ...
    const newLogicalMonitors = logicalMonitors.map((logicalMonitor) => {
      ...
      const newSpecs = monitorSpecs.map((monitorSpec) => {
        ...
        if (monitorInfo) {
          ...
          modeId = currentMode[0];

          const supportedScales = currentMode[5];

          if (!supportedScales.includes(scale)) {
            // snap to nearest supported scale
            appliedScale = supportedScales.reduce((closest, scl) =>
              Math.abs(scl - scale) < Math.abs(closest - scale) ? scl : closest,
            );

            if (this._settings.get_boolean("show-unsupported-notification"))
              Main.notify(
                this._("Display Scale Not Supported"),
                sprintf(
                  this._(
                    "The scale %s is not supported for %s. Using the nearest supported scale %s instead. Supported: %s.",
                  ),
                  scale,
                  connector,
                  appliedScale,
        ...
        return { connector, modeId, appliedScale };
      });

      // all monitors in a logical monitor share one scale value, so we can just use the first spec's resolved scale
      const finalScale = newSpecs[0]?.appliedScale ?? scale;
      const specsForApply = newSpecs.map(({ connector, modeId }) => [
        connector,
        modeId,
        {},
      ]);

      return [x, y, finalScale, transform, primary, specsForApply];
    });
