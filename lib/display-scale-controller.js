...
export class DisplayScaleController {
  ...
  async setScale(scale, { persistent = false } = {}) {
    ...
    if (typeof scale !== "number" || !Number.isFinite(scale) || scale <= 0)
      throw new RangeError(`The scale ${scale} is invalid.`);
        ...
        if (monitorInfo) {
          ...
          if (!supportedScales.includes(scale)) {
            ...
            if (this._settings.get_boolean("show-unsupported-notification"))
              Main.notify(
                "Display Scale Not Supported",
                `The scale ${scale} is not supported for ${connector}. Using the nearest supported scale ${appliedScale} instead. Supported: ${supportedScales.join(", ")}.`,
              );
          }
        }
